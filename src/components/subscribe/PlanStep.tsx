...
const HIGHLIGHT: SignupPlan = "pro";
...
/** Step 1: two plan cards. Pro is the recommended one and carries the neon edge. */
export function PlanStep({
  ...
  currency: Currency;
  selected: SignupPlan | null;
  onChoose: (plan: SignupPlan) => void;
}) {
  ...
  return (
    <div>
            ...
            >
              ...
              <p className="mt-5 flex items-baseline gap-2 tabular-nums">
                <span className="text-[2.5rem] leading-none font-semibold tracking-tight">
                  {money(priceMinor(plan, currency), currency)}
                </span>
              ...
              >
                {t("plan.choose", { plan: t(`plan.names.${plan}`) })}
              </button>
            </li>
          );
        })}
      ...
      {currency !== "AOA" && (
        <p className="mt-5 text-[13px] leading-relaxed text-ink-muted">{t("plan.rates")}</p>
      )}
    </div>
  );
}
